...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { admin_id, student_id, suspend } = await req.json();
    ...
    const { data, error } = await supabase.from("students").update({ is_active: !suspend }).eq("id", student_id).select("id, username, full_name, is_active").single();
    if (error) return errorResponse("Failed: " + error.message, 500);
    return json({ success: true, student: data, action: suspend ? "suspended" : "activated" });
